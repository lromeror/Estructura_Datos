...
public class LinkedList<E> implements List<E> {
    ...
    @Override
    public List<E> findIntersection(List<E> anotherList, Comparator<E> cmp) {//Le pasamos como parametro un comparador
        //Pero el comprador debe estar prametrzado para saber que son estudiante que le pasaremos
        List<E> results = new LinkedList<>();
        for (E e1 : this) {//Para hacer esto necesitamos el iterador
            for (E e2 : anotherList) {
                if (cmp.compare(e1, e2) == 0) { //Recordar que este comparador
                    //Osea sabemos que vamos a comparar 
                    //Si es igual a Cero, los objetos son iguales
                    //cmp.compare ya nos da comparada esto lo hacemos para no usar el equals
                    results.addLast(e2);
                }
            }
        }
